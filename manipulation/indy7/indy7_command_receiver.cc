...
Indy7CommandReceiver::Indy7CommandReceiver(int num_joints, int num_fingers)
    : num_joints_(num_joints),
      num_fingers_(num_fingers) {
  message_input_ = &DeclareAbstractInputPort(
      "lcmt_indy7_command", Value<lcmt_indy7_command>());
  ...
  position_measured_or_zero_ = &DeclareCacheEntry(
      ...
      &Indy7CommandReceiver::CalcPositionMeasuredOrZero,
  ...
  groomed_input_ = &DeclareCacheEntry(
      "groomed_input", &Indy7CommandReceiver::CalcInput,
  ...
  commanded_position_output_ = &DeclareVectorOutputPort(
      ...
      &Indy7CommandReceiver::CalcPositionOutput,
      {groomed_input_->ticket()});

  commanded_velocity_output_ = &DeclareVectorOutputPort(
      "velocity", num_joints + num_fingers,
      &Indy7CommandReceiver::CalcVelocityOutput,
      {groomed_input_->ticket()});

  time_output_ = &DeclareVectorOutputPort(
      "time", 1, &Indy7CommandReceiver::CalcTimeOutput,
      {groomed_input_->ticket()});
}

void Indy7CommandReceiver::CalcPositionMeasuredOrZero(
...
void Indy7CommandReceiver::LatchInitialPosition(
    const Context<double>& context,
...
void Indy7CommandReceiver::DoCalcNextUpdateTime(
    const Context<double>& context,
...
void Indy7CommandReceiver::CalcInput(
  const Context<double>& context, lcmt_indy7_command* result) const {
  if (!get_message_input_port().HasValue(context)) {
    throw std::logic_error("Indy7CommandReceiver has no input connected");
  }

  // Copy the input value into our tentative result.
  *result = get_message_input_port().Eval<lcmt_indy7_command>(context);
  ...
  if (lcm::AreLcmMessagesEqual(*result, lcmt_indy7_command{})) {
  ...
  if (result->num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
        "Indy7CommandReceiver expected num_joints = {}, but received {}",
        num_joints_, result->num_joints));
  }

  if (result->num_fingers != num_fingers_) {
    throw std::runtime_error(fmt::format(
        "Indy7CommandReceiver expected num_fingers = {}, but received {}",
        num_fingers_, result->num_fingers));
  }
}

void Indy7CommandReceiver::CalcPositionOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = groomed_input_->Eval<lcmt_indy7_command>(context);
...
void Indy7CommandReceiver::CalcVelocityOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = groomed_input_->Eval<lcmt_indy7_command>(context);
  if (message.num_joints != num_joints_) {
    throw std::runtime_error(fmt::format(
...
void Indy7CommandReceiver::CalcTimeOutput(
    const Context<double>& context, BasicVector<double>* output) const {
  const auto& message = groomed_input_->Eval<lcmt_indy7_command>(context);
