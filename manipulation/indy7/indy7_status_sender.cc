...
Indy7StatusSender::Indy7StatusSender(int num_joints, int num_fingers)
  ...
  DeclareAbstractOutputPort(
      "lcmt_indy7_status", &Indy7StatusSender::CalcOutput);
}

void Indy7StatusSender::CalcOutput(
    const systems::Context<double>& context, lcmt_indy7_status* output) const {
  ...
  lcmt_indy7_status& status = *output;
  ...
  for (int i = 0; i < num_joints_; ++i) {
    ...
    // It seems like the Indy7 (at least for some models/firmwares) reports
    // half of the actual angular velocity.  Fix that up here.  Note
    // bug-for-bug compatibility implemented in Indy7StatusReceiver.
